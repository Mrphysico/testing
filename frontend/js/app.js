...
function App() {
  ...
  const notify = (message) => {
    ...
    if (typeof text !== "string") {
      text = text?.message || (text ? JSON.stringify(text) : "An error occurred");
    }
    setToast(text);
...
function Login(props) {
  ...
  async function handleSendOtp(e) {
    e.preventDefault();
    const name = (publicSignup.name || "").trim();
    const email = (publicSignup.email || "").trim();
    const phone = (publicSignup.phone || "").trim();
    const plate = (publicSignup.plate_number || "").trim().toUpperCase();
    const password = publicSignup.password || "";

    if (!name || !email || !password || !phone || !plate) {
      notify("Please fill in all fields (Name, Email, Password, Phone, Plate Number).");
      return;
    }
    ...
    try {
      ...
      notify(`Verification OTP sent to ${email} and ${phone}`);
    } catch (err) {
      notify(err.message);
    } finally {
      setLoading(false);
  ...
  async function handleVerifyOtp(e) {
    ...
    if (!code || code.length < 4) {
      notify("Please enter the 6-digit verification OTP code.");
      return;
    }
    setLoading(true);
    try {
      const data = await api("/auth/verify-signup-otp", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          name: (publicSignup.name || "").trim(),
          email: (publicSignup.email || "").trim(),
          password: publicSignup.password,
          phone: (publicSignup.phone || "").trim(),
          plate_number: (publicSignup.plate_number || "").trim().toUpperCase(),
          otp_code: code
        })
      });
      localStorage.setItem("token", data.access_token);
              ...
              signupStep === "details" ? (
                ...
                <form onSubmit={handleSendOtp} className="space-y-3">
                  ...
                  <p className="text-[11px] font-semibold text-slate-500">
                    * Enter any family vehicle plate and emergency phone. If not already in the system, it will be automatically registered for you.
                  </p>
                  ...
                  >
                    <Icon name="send" /> {loading ? "Processing..." : "Send Verification OTP"}
                  </button>
                </form>
              ) : (
